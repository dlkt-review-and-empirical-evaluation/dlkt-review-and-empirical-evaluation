...
def onehot_(input):
    a = np.array(input)
    af = a.flatten()
    oh = np.zeros([af.size, af.max() + 1])
    oh[np.arange(af.size), af] = 1
    return oh
        ...
        if 'a' in active_features:
            ...
            # Counts
                ...
                if 'ic' in active_features:
                    ...
                    item_ids_onehot = onehot_(item_ids)
...
if __name__ == "__main__":
    ...
    data_path = os.path.join('../data/preprocessed', args.dataset)
    df = pd.read_csv(os.path.join(data_path, 'glr-preprocessed.csv'), sep="\t")
